...
- `rate_limit` が来ても作業が止まったとは限らない。モデルのフォールバックで続行できた場合にも発火する
:::
...
### `rate_limit` は「使えなくなった」とは限らない

ここに落とし穴があります。`rate_limit` で発火したからといって、そのアカウントで作業が止まったとは限りません。

上限に当たっても、モデルのフォールバックが働いて応答がそのまま続くことがあります。この場合ユーザーの画面には何も表示されませんが、**内部的にはエラーメッセージが 1 件記録され、`StopFailure` は発火します**。

つまり素直に「`rate_limit` が来たら切り替える」と書くと、実際には会話が続いているのに新しいペインが生えてきます。

見分けるには `last_assistant_message` を見ます。フォールバックで処理されたケースでは、ユーザーに見せないための定型文字列が入っています。

```javascript
// 応答が続いているので、切り替えてはいけない
if (payload.last_assistant_message?.trim() === 'No response requested.') {
  process.exit(0);
}
```

「上限」にも種類があります。5 時間単位の枠、7 日単位の枠、Opus だけの週次枠が別々に管理されていて、レスポンスヘッダーで区別されています。フックからはこの区別が直接は見えないので、**クールダウンを 60 分に固定すると週次の枠に当たったときに短すぎます**。週次枠まで踏むなら、クールダウンは日単位に伸ばす前提で設計しておくのが安全です。
...
履歴は `<設定ディレクトリ>/projects/<作業ディレクトリから導いた名前>/<セッション ID>.jsonl` に置かれています。

```console
$ ls ~/.claude/projects/
-Users-you-Develop-myapp
-Users-you-Develop-another-project
```

作業ディレクトリのパスを変換した名前になっています。ここで「`/` と `.` を `-` に置換すればいい」と早合点すると失敗します。実際の規則は**英数字以外をすべて `-` に置換する**もので、さらに名前が長くなりすぎる場合はハッシュが付きます。スペースやアンダースコアを含むパスで、自作の変換とずれます。

```bash
# ❌ 一見それらしいが、スペースや _ を含むパスで一致しない
key=$(pwd | sed 's|/|-|g; s|\.|-|g')
```

変換規則を推測して再実装するより、**実物を探すほうが確実**です。セッション ID は UUID なので、1 段掘れば一意に見つかります。

```bash
# 履歴ディレクトリ全体ではなく、そのセッションだけを渡す
src=$(find ~/.claude/projects -maxdepth 2 -name "$SESSION_ID.jsonl" | head -1)
dest="$HOME/.claude-work/projects/$(basename "$(dirname "$src")")"
mkdir -p "$dest"
ln -s "$src" "$dest/"
```

切り替え先も同じ作業ディレクトリから同じ名前を導くので、**元のディレクトリ名をそのまま持ち込めば一致します**。フックが `transcript_path` を渡してくれるときは、探索すら不要です。
...
| `maxSwitchesPerHour` | 4（回/時） | 全体の上限。暴走を止める最後の砦 |

既定を `notify`（通知のみ）にしているのは、いきなり全自動にすると挙動が読めないためです。しばらく通知で様子を見て、納得してから `auto` に上げる想定にしています。

`maxSwitchesPerHour` が最後の砦です。前述のとおり `rate_limit` は必ずしも「作業が止まった」を意味しないので、想定外のタイミングで発火する可能性が残ります。回数の天井だけは、条件を問わず効くようにしておきます。
...
`CLAUDE_CONFIG_DIR` はディレクトリを指すだけの変数に見えて、実際には「どの Keychain 項目を見るか」まで決めています。既定パスを明示的に書いた瞬間に参照先が変わる、というのが今回のハマりどころでした。

既定アカウントには変数を付けない。付けるときは文字列を固定する。あとは `~/.claude` を symlink で差し替えないこと。この 3 つを守れば、設定ディレクトリを分けるだけで済みます。

自動切り替えのほうは、`rate_limit` が必ずしも「そのアカウントで作業が止まった」を意味しない点が効いてきます。フックの出力は読まれず、上限の種類もわからない以上、判断材料は限られています。クールダウンと回数の天井を最初から入れておくのが現実的でした。
